...
try {
	...
	if ($numParticipantes > 0) {
		...
		$htmlOutput = "<div class='table-responsive'>
            ...
            <thead>
                <tr>
                    <td colspan='7' align='center' style='color: #fff; text-transform: uppercase;'><b>Participantes Confimados Pago</b></td>
                </tr>
                <tr style='background: linear-gradient(180deg, #FF8A32 0%, #FFB4B4 100%); color: #000;'>
                    ...
                    <th class='text-left' style='width: 120px;'>Email</th>
                    <th class='text-center' style='width: 80px;'>Data</th>
                    <th class='text-right' style='width: 100px;'>Valor pago</th>
                </tr>
            </thead>
            <tbody>";
		...
		foreach ($participantes as $row) {
			...
			$htmlOutput .=  "<tr>
				...
				<td class='text-left' >" . htmlspecialchars($row['email_insc'], ENT_QUOTES, 'UTF-8') . "</td>		
				<td class='text-center''>{$dataCadastroFormatada}</td>
				<td class='text-right'>R$ " . htmlspecialchars($row['valor_insc'], ENT_QUOTES, 'UTF-8') . "</td>
			</tr>";
		}
