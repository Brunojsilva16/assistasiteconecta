...
$status = trim(strip_tags($raw_status));
$motivo = trim(strip_tags($raw_motivo));
...
if (empty($status) || empty($tabela) || $id === false) {
...
try {
    ...
    $sql = "UPDATE `$tabela` SET status_insc = ?, motivo_insc = ? WHERE id_insc = ?";
    $params = [
        $status, // A string sanitizada e segura
        $motivo, // A string sanitizada e segura
        $id
    ];
    ...
    if ($affectedRows > 0) {
        ...
        $response['message'] = 'Status da inscrição atualizado com sucesso!';
    } else {
