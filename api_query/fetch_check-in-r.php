...
try {
	...
	if ($numParticipantes > 0) {
		...
		$htmlOutput = "<div class='table-responsive'>
			...
			<thead>
				<tr>
					<td colspan='5' align='center' style='color: #fff; text-transform: uppercase;' ><b>Tabela dos Participantes com Check-in Confirmado<b> </td>
				</tr>
				<tr>
		...
		foreach ($participantes as $row) {
			...
			$botao_baixar = '<button type="button" 
							onclick="baixarCertificado(this);" 
							class="button button-secondary" data-nome="' . $row['nome_insc'] . '"
							data-evento="Workshop Manejo Clínico dos Transtornos por Uso de Substâncias"
							data-palestrante="Dr. Paulo de Tarso Melo"
							data-data="no dia 20 de setembro de 2025"
							data-cargah="4"
							data-local=" ">
							<i class="fas fa-download"></i> Baixar
						</button>';

			// Verifica se a coluna 'check_in_email' existe e se o valor é maior que 0
			if (isset($row['check_in_email']) && $row['check_in_email'] > 0) {
				// Se o e-mail JÁ FOI ENVIADO, cria o botão desabilitado
				$botao_enviar = '<button type="button" class="button button-disabled" disabled>
                        <i class="fas fa-check-circle"></i> Enviado!
                </button>';
			} else {
				// Se o e-mail AINDA NÃO FOI ENVIADO, cria o botão normal
				$botao_enviar = '<button type="button" 
                        onclick="getCertificado(this);" 
                        class="button button1" 
                        data-id="' . $row['id_insc'] . '" 
                        data-nome="' . $row['nome_insc'] . '" 
                        data-email="' . $row['email_insc'] . '"
						data-nomeTable="' . $nomeTabela . '"
                        data-evento="Workshop Manejo Clínico dos Transtornos por Uso de Substâncias"
                        data-palestrante="Dr. Paulo de Tarso Melo"
                        data-data="no dia 20 de setembro de 2025"
                        data-cargah="4"
                        ...
                        <i class="fas fa-check"></i> Enviar Certificado
                    </button>';
			}

			$acao .= " " . $botao_enviar . " " . $botao_baixar;
