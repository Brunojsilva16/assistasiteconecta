...
try {
    ...
    if ($numParticipantes > 0) {
        ...
        $htmlOutput = "<div class='table-responsive'>
            ...
            <thead>
                <tr>
                    <td colspan='6' align='center' style='color: #fff; text-transform: uppercase;'><b>Participante cancelado ou desistente<b>
                    </td>
                </tr>
                <tr style='background: linear-gradient(180deg, #FF8A32 0%, #FFB4B4 100%); color: #000'>
                    ...
                    <th class='text-left' style='width: 150px;'>Motivo</th>
        ...
        foreach ($participantes as $row) {
            ...
            if (isset($_SESSION['acesso']) && $_SESSION['acesso'] > 3) {
                $acao = "<button 
                        type='button'
                        class='btn btn-outline-success btn-sm btn_crachar' 
                        onclick='get_crachParam(this);' 
                        data-id='{$idInscEscapado}' 
                        data-nome='{$nomeInscEscapado}'>
                        <i class='fas fa-check'></i> baixar
                     </button>";
            } else {
                $acao = "Acesso não permitido";
            }
            ...
            $htmlOutput .=  "<tr>
                    ...
                    <td class='text-left'>" . htmlspecialchars($row['motivo_insc'], ENT_QUOTES, 'UTF-8') . "</td>
