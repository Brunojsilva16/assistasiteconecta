...
$anexoTables = ['inscricao', 'te_inscricao'];
...
$vdoc = "";
$respon = "";
$colonia_terapeutica = false;

if (in_array($nomeTabela, $anexoTables)) {
  $anexo = true;
  $colsp = '8';
  $th = "<th class='text-center' style='width: 60px;'>Anexo</th>";
} else {
  $anexo = false;
  $colsp = '7';
  $th = "";
...
if ($nomeTabela === 'colonia_te') {
  $anexo = false;
  $colsp = '8';
  $respon = "<th class='text-center' style='width: 220px;'>Responsável</th>";
  $colonia_terapeutica = true;
}
  ...
  if ($numParticipantes > 0) {
    ...
    $htmlOutput = "<div class='table-responsive'>
      ...
      <table class='table table-bordered table-striped' style='margin-top:20px;'>
        <thead>
        <tr>
          <td colspan='{$colsp}' align='center' style='color: #fff; text-transform: uppercase;'><b>Todos Participantes Inscritos</b></td>
        </tr>
    ...
    foreach ($participantes as $row) {
      ...
      if ($anexo) {
        // --- Lógica do Link de Anexo ---
        $doc_path = '../assets/uploads/' . $row['doc_insc'];
        if (!empty($row['doc_insc']) && file_exists($doc_path)) {
          $viewDoc = "<a href='./assets/uploads/{$row['doc_insc']}' target='_blank' class='btn anexo'><i class='fas fa-file-download' aria-hidden='true'></i></a>";
          $vdoc = "<td class='text-center'>{$viewDoc}</td>";
        } else {
          $viewDoc = "<i class='fas fa-times'>";
          $vdoc = "<td class='text-center'>{$viewDoc}</td>";
        }
      }

      if ($colonia_terapeutica) {
        $responsavel = '<td class="text-center">' . $row['responsavel_insc'] . '</td>';
      }
      ...
      if ($row['status_insc'] == 'Confirmado') {
        $statusClass = 'class="status active"';
        $icon = '<i class="fas fa-check-circle"></i>';
      } else if ($row['status_insc'] == 'Isento') {
        $statusClass = 'class="status isento"';
        $icon = '<i class="fas fa-check-circle"></i>';
      } else if ($row['status_insc'] == 'Cancelado') {
      ...
      $htmlOutput .= $responsavel ?? '';

      $htmlOutput .= "
        <td class='text-left'>" . htmlspecialchars($row['categoria_insc'], ENT_QUOTES, 'UTF-8') . "</td>
        <td class='text-left' style='white-space: nowrap; width: 180px;'>" . htmlspecialchars($row['telefone_insc'], ENT_QUOTES, 'UTF-8') . "</td>
        <td class='text-center'><span {$statusClass}>" . $icon . htmlspecialchars($row['status_insc'] ?? 'Pendente', ENT_QUOTES, 'UTF-8') . "</spon></td>
