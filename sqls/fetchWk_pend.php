...
$sql = "SELECT * FROM `workshop` WHERE status_insc is null ORDER BY `datacad` DESC";
...
while ($row = $resultConut->fetch(PDO::FETCH_ASSOC)) {
	...
	$acao = ($row['status_insc'] != 'Confirmado') ? '<button class="btn btn-outline-success" onclick="getWk_param(this);" data-p="1" data-v="Confirmado" data-id="' . $row['id_insc'] . '"><i class="fas fa-solid fa-check"></i></button> <button type="button" class="btn btn-outline-danger" onclick="getWk_param(this);" data-p="1" data-v="Cancelado" data-id="' . $row['id_insc'] . '"><i class="fas fa-times" aria-hidden="true"></i>
</button>' :
		'<button type="button" class="btn btn-outline-danger" onclick="getWk_param(this);" data-p="1" data-v="Cancelado" data-id="' . $row['id_insc'] . '"><i class="fas fa-times" aria-hidden="true"></i>
</button>';
